...
```python
# ✅ CORRECT: Concurrent read-only queries (independent queries, same session OK for reads)
async def get_dashboard_data(user_id: UUID, session: AsyncSession):
    # These are read-only and don't depend on each other — run in parallel
...
async def transfer_credits(
    ...
    # Lock in consistent order using select().with_for_update()
